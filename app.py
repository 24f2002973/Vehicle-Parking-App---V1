...
def init_db_command():
    ...
    if not admin:
        admin = User(
            username='admin',
            email='parkingapp.admin@gmail.com',
...
def manage_lots():
    ...
    if request.method == 'POST':
        ...
        if ParkingLot.query.filter_by(name=name).first():
            flash('Lot name already exists', 'danger')   
        else:
...
def edit_lot_spots(lot_id):
    ...
    if request.method == 'POST':
        new_max_spots = int(request.form['new_max_spots'])
        current_spots = ParkingSpot.query.filter_by(lot_id=lot.id).count()

        if new_max_spots > lot.max_spots:
            for spot_num in range(lot.max_spots + 1, new_max_spots + 1):
                new_spot = ParkingSpot(spot_number=spot_num, status='A', lot_id=lot.id)
                db.session.add(new_spot)
            flash(f'Added {new_max_spots - lot.max_spots} new spot(s) to {lot.name}', 'success')
            ...
            for spot in extra_spots:
                if spot.status == 'A' and not spot.reservations:
                    db.session.delete(spot)
                else:
                    flash(f'Cannot delete spot {spot.spot_number}: it is occupied or has reservations', 'warning')
            flash(f'Removed some extra spots from {lot.name}.', 'info')
        
        lot.max_spots = new_max_spots
        db.session.commit()
        return redirect(url_for('manage_lots'))
    
    return render_template('edit_spots.html', lot=lot)
